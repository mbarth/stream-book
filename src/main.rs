...
async fn main() -> anyhow::Result<()> {
    ...
    // Wrap the snapshot receiver in an Arc<Mutex<>>
    let shared_snapshot_receiver = Arc::new(Mutex::new(snapshot_receiver.resubscribe()));
    let actix_future = HttpServer::new(move || {
        // Clone the snapshot receiver
        let cloned_snapshot_receiver = Arc::clone(&shared_snapshot_receiver);
        App::new()
    ...
    // Building Tonic gRPC server with an order book aggregator
    let tonic_future = {
        let order_book_aggregator = StreamBookAggregator { snapshot_receiver };
